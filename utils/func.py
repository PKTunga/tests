...
dynamo_settings_table_name = 'counterSettingTable' 


dynamo_settings_table_name = 'SettingsTable' 
...
def get_counter():
    ...
    table = dynamo_client.Table(dynamo_settings_table_name)
...
def query_dynamo(instance_id, ip_addr, cross_acc_id, event):
    ...
    if response['Count'] == 1:
        ...
        new_serial_number = datetime.datetime.now().strftime("%H%M%S")
        ...
        if current_dt > db_date_dt:
            ...
            serial_update = table_name.update_item(
                ...
                ExpressionAttributeValues={
                    # ":serial_number": 11111,
                    ":serial_number": int(new_serial_number),
            ...
            hosted_record_name = 'awsproxy' + dd_mm +  get_counter() + str(serial_number) + domain_name
            increment_counter()
            hosted_zone_id = hosted_zone_record_id
