...
@method_decorator(rate_limit(action="referral_signup"), name="dispatch")
class ReferralSignupView(
    ...
    template_name = "account/referral_signup." + app_settings.TEMPLATE_EXTENSION
    form_class = SignupForm
    redirect_field_name = "next"
    success_url = None
    ...
    @sensitive_post_parameters_m
    def dispatch(self, request, *args, **kwargs):
        return super(ReferralSignupView, self).dispatch(request, *args, **kwargs)
    
    def get(self, request, *args, **kwargs):
        """Handle GET requests: instantiate a blank version of the form."""
        token = request.GET.get("token", None)
        return self.render_to_response(self.get_context_data(token))
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
        # token = request.GET.get("token", None)
        # relation = ReferralRelationship.objects.get(refer_token=token)
        # if isinstance(relation, ReferralRelationship):
        #     relation.invited = request.user
        #     relation.save()
        form = self.get_form()
        if form.is_valid():
    ...
    def get_context_data(self,token=None, **kwargs):
        ret = super(ReferralSignupView, self).get_context_data(**kwargs)
        form = ret["form"]
        email = self.request.session.get("account_verified_email")
        if email:
            email_keys = ["email"]
...
referral_signup = ReferralSignupView.as_view()
